...
> = {
  ...
  DB_TIMEOUT: {
    ...
    suggestion:
      "The database may be in sleep mode or there may be network issues",
    fixable: true,
  },
  DB_HOST_NOT_FOUND: {
  ...
  DB_SSL_ERROR: {
    ...
    suggestion:
      "For Neon/Supabase, try using non-pooled connection or check SSL configuration",
    fixable: true,
  },
  ...
  SQL_NO_SELECT: {
    ...
    suggestion:
      "Use SELECT to read data. INSERT, UPDATE, DELETE are not permitted",
    fixable: false,
  },
  ...
  SQL_DEEP_NESTING: {
    ...
    suggestion:
      "Use CTEs (WITH clause) to simplify or break into multiple queries",
    fixable: true,
  },
  SQL_INVALID_COLUMN: {
  ...
  CHART_NO_DATA: {
    ...
    suggestion:
      "Ensure your query returns results. Try adding data or adjusting filters",
    fixable: true,
  },
  CHART_NO_NUMERIC_COLUMN: {
    message: "No numeric column found for visualization",
    suggestion:
      "Rewrite query with aggregations: COUNT(*), SUM(column), AVG(column), etc.",
    fixable: true,
  },
 ...
 */
export function createDatabaseError(error: unknown): ErrorResponse {
 ...
 */
export function createSQLError(error: unknown, query?: string): ErrorResponse {
  const message = error instanceof Error ? error.message : String(error)
  const code = (error as { code?: string })?.code || ""
