...
import type { SQLQueryResult, XAxisConfig, YAxisConfig, ChartOptions } from '../types.js';
...
describe('Data Processors', () => {
  ...
  it('should process bar chart', () => {
    ...
    const options: ChartOptions = { legend: true, stacked: false, horizontal: false, showDataLabels: false };
  ...
  it('should process pie chart', () => {
    const yAxis: YAxisConfig[] = [{ column: 'value' }];
    const options: ChartOptions = { legend: true, stacked: false, horizontal: false, showDataLabels: false, limit: 7, sort: 'desc' };
  ...
  it('should count data points', () => {
    const series = [{ name: 'A', data: [{ x: 1, y: 10, label: '1:10' }], color: '#ff0000' }];
  ...
  it('should throw on invalid column', () => {
    ...
    expect(() => processXYChart(mockData, 'bar', xAxis, yAxis, options)).toThrow();
  });
});
