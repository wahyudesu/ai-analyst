...
export function validateSQLQuery(query: string): ValidationResult {
  ...
  if (!trimmedQueryUpper.startsWith("SELECT")) {
    return {
      isValid: false,
      error:
        "Only SELECT queries are allowed for security. Queries starting with INSERT, UPDATE, DELETE, DROP, ALTER, etc. are not permitted.",
    }
  }
  ...
  if (trimmedQuery.includes(";")) {
    ...
    if (semicolonCount > 1 || lastChar !== ";") {
      return {
        isValid: false,
        error:
          "Multi-statement queries are not allowed. Only a single trailing semicolon is permitted.",
      }
    }
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(query)) {
      return {
        isValid: false,
        error:
          "Query contains potentially dangerous patterns that are not allowed.",
      }
    }
  }
  ...
  if (!/\bLIMIT\b/i.test(query) && !/\bFETCH\s+FIRST\b/i.test(query)) {
    warnings.push(
      "Query has no LIMIT clause. This could return a large number of rows. Consider adding LIMIT."
    )
  }
  ...
  const unquotedStringPattern =
    /=\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:WHERE|AND|OR|LIMIT|GROUP|HAVING|ORDER|$)/g
  let match
  while ((match = unquotedStringPattern.exec(query)) !== null) {
    if (
      match[1] &&
      !["NULL", "TRUE", "FALSE"].includes(match[1].toUpperCase())
    ) {
      warnings.push(
        `Possible unquoted string literal: "${match[1]}". This might indicate an injection attempt or malformed query.`
      )
    }
  }
