...
Use this after generating a query to help users understand what will happen.`,
  ...
  execute: async ({ query }) => {
    ...
    if (upperQuery.includes("SELECT")) {
      explanation.push(
        "**This is a SELECT query** - it retrieves data from the database."
      )
    }
    ...
    if (upperQuery.includes("WHERE")) {
      explanation.push(
        "**Filtering:** Results are filtered using WHERE clause."
      )
    }
    ...
    if (aggregations.length > 0) {
      explanation.push(
        `**Aggregations:** ${aggregations.join(", ")} - calculates summary values.`
      )
    }
    ...
    if (upperQuery.includes("GROUP BY")) {
      explanation.push(
        "**Grouping:** Results are grouped by one or more columns."
      )
    }
    ...
    if (
      joinCount > 3 ||
      (aggregations.length > 0 && upperQuery.includes("ORDER BY"))
    ) {
      complexity = "high"
      reasons = joinCount > 3 ? ["many joins"] : ["aggregation with sorting"]
    }
    ...
    if (upperQuery.includes("SELECT *")) {
      suggestions.push(
        "Consider specifying explicit columns instead of SELECT * for better performance."
      )
    }
