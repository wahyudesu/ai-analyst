...
function analyzeQueryComplexity(query: string): QueryAnalysis {
  ...
  const joinMatches = upperQuery.match(
    /\b(?:INNER\s+|LEFT\s+|RIGHT\s+|FULL\s+|CROSS\s+)?JOIN\b/g
  )
  ...
  const hasAggregate = /\b(COUNT|SUM|AVG|MIN|MAX|STDDEV|VARIANCE)\s*\(/i.test(
    query
  )
 ...
 */
function detectInjectionPatterns(query: string): {
  detected: boolean
  pattern?: string
  suggestion?: string
} {
...
export function validateSQLQueryV2(query: string): ValidationResult {
  ...
  if (semicolonCount > 1) {
    return {
      ...
      suggestions: [
        ...
        "Use a subquery instead of multiple statements",
      ],
    }
  }
  ...
  if (injectionCheck.detected) {
    return {
      ...
      suggestions: [
        injectionCheck.suggestion || "Review and simplify the query",
      ],
    }
  }
      ...
      if (/\S\s+--/.test(line) || /\S\s+\/\*/.test(line)) {
        return {
          ...
          suggestions: [
            ...
            "Ensure query is a single statement",
          ],
        }
      }
    }
  ...
  if (complexityScore > 50) {
    warnings.push(
      `Query complexity score: ${complexityScore}/100. This may be slow.`
    )
  }
  ...
  if (analysis.subqueryDepth > 3) {
    return {
      ...
      suggestions: [
        "Use CTEs (WITH clause) to simplify",
  ...
  if (
    !/\bLIMIT\b/i.test(trimmedQuery) &&
    !/\bFETCH\s+FIRST\b/i.test(trimmedQuery)
  ) {
    if (analysis.estimatedCost === "high") {
      suggestions.push("Add LIMIT clause to prevent large result sets")
    } else {
      warnings.push(
        "Query has no LIMIT clause. Consider adding one for performance."
      )
    }
  }
  ...
  if (/\bSELECT\s+\*\b/i.test(trimmedQuery)) {
    warnings.push(
      "SELECT * can return unnecessary data. Specify explicit columns."
    )
  }
    ...
    if (trimmedQueryUpper.includes(func.toUpperCase())) {
      return {
        ...
        suggestions: [
          ...
          "Contact administrator if this is needed",
        ],
      }
    }
  }
  ...
  if (/\b(COPY|EXECUTE)\b/i.test(trimmedQuery)) {
    return {
      ...
      suggestions: [
        "Use regular SELECT queries",
        "Contact administrator for data export",
      ],
    }
  }
