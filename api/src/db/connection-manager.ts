...
type ConnectionResult =
  | { success: true; pool: Pool; connectionTime: number }
    ...
    try {
      ...
      for (const param of unsupportedParams) {
        if (url.searchParams.has(param)) {
          ...
          console.warn(
            `Removed unsupported parameter '${param}' from connection string`
          )
        }
      }
      ...
      throw new Error(
        `Invalid connection string: ${err instanceof Error ? err.message : "unknown"}`
      )
    }
  }
  ...
  async getConnection(connectionString: string): Promise<ConnectionResult> {
    try {
      ...
      if (existingTimeout) {
        ...
        console.log(
          `Reusing existing connection to: ${config.database}@${config.host}`
        )
      }
      ...
      if (message.includes("certificate") || message.includes("SSL")) {
        return {
          success: false,
          error:
            `SSL certificate error. If using Neon/Supabase pooler, try:\n` +
