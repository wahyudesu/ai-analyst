...
const db1 = 'postgresql://neondb_owner:npg_lqdYNJuIO08a@ep-proud-mouse-aijyu7tu-pooler.c-4.us-east-1.aws.neon.tech/neondb?sslmode=require';
const db2 = 'postgresql://postgres.mmxbumvufqzhxunpjjuy:NoLIuOGKa76QrP8o@aws-1-ap-southeast-1.pooler.supabase.com:5432/postgres';
...
async function getSchema(connectionString: string, name: string) {
  ...
  try {
    ...
    const tables: Record<string, { columns: Array<{ column: string; type: string; nullable: boolean; default: string | null }> }> = {};
...
async function main() {
  ...
  console.log(`  Neon (AI-Analyst):    ${Object.keys(schema1.tables).length} tables`);
  console.log(`  Supabase (Aiman):    ${Object.keys(schema2.tables).length} tables`);
